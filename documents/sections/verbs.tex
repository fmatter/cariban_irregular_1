...
As seen in \cref{sec:pekodian,sec:taranoan,tab:comepara}, this division can exist within a single paradigm.
...
\akuriyo reflects \rc{əe} as \obj{ee}, with \obj{eepɨ} being betrayed as not a reflex of \rc{epɨ} by the otherwise unexpected vowel length \parencite[114--115]{meira1998proto}.
...
In \kalina \parencite[509--510]{meira2010origin}, \rc{e}-initial transitive verbs detransitivized with \rc{ət-} underwent the same development (also yielding vowel length), while \rc{əte} > \rc{əə} is irregular at least in \kaxui and \panare.
...
Putative \PC \rc{jəpɨ} would yield \rc{epɨ} via the two widespread sound changes \phonl{\rc{ə}}{\rc{e}}{\rc{j}} and \phonl{\rc{j}}{∅}{\#} \parencite{meira2010origin}.
...
Thus, it does not explain \kaxui \obj{ehɨ}, nor the absence of the sequence \rc{jə} in \panare and \mapoyo-\yawarana.
%
...
However, unlike \qu{to sleep}, bare \rc{epɨ} \qu{to come} apparently already was an \gl{s_a_} verb (although its reflexes in languages with split-\gl{s} mostly co-occur with reflexes of \rc{ət-epɨ}).
...
The verb shows traits of both classes in \wayana, making it a \dbqu{mixed} verb in a synchronic analysis.
...
These patterns point to \qu{to go down} as a regular \gl{s_p_} verb in pre-\wayana, but partially switching to the \gl{s_a_} class by taking a \gl{1+2}\gl{s_a_} prefix and the \gl{s_a_} class marker.
This in turn implies that (inflectionally defined) \gl{s_a_} reflexes in other languages fully switched from \gl{s_p_} to \gl{s_a_}.
...
These forms have a reflex of the transitivizer \rc{en-}, occurring with \gl{s_p_} verbs in \PC \parencite{gildea2019overview}.
...
In some cases, the verb does not occur, or at least not in a first person form (–), in others that form is unknown (?), and the question of affectedness is often not meaningfully answerable (\textsc{n/a}) for \qu{to go down} \pcref{sec:godown}.
...
\cref{sec:motivations} will explore possible explanations for the same 1-7 verbs retaining their old \gl{1}\gl{s_a_} marker in 6 independent developments, while a plethora of regular \gl{s}\textsubscript{(\gl{a})} verbs take on innovative markers.
