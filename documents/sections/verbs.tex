\section{Resistant verbs in a comparative perspective}
\label{sec:verbs}
In \cref{sec:extensions}, six distinct extensions of personal prefixes into \gl{1}\gl{s_a_} territory were introduced, with certain verbs resisting the innovation.
...
In this section, I present these verbs from a comparative perspective and discuss their reconstructibility.
Attentive readers will have noticed that certain verbs reoccurred in these lists, not only matching in meaning but also largely in form.
In this section, I will discuss these verbs from a comparative perspective.
\cref{sec:be} treats the copular verb \rc{eti}/\obj{a(p)} \qu{to be}, \cref{sec:say} \rc{ka(ti)} \qu{to say}, \cref{sec:go} \qu{ɨtə(mə)} \qu{to go}, and \cref{sec:come} \rc{(ət)jəpɨ} \qu{to come}; these are all verbs that \textcite{gildea2007greenberg} reconstructed as \gl{s_a_} verbs that were not derived from transitive verbs.
...
\label{sec:be}
For a comprehensive comparative overview for this verb, I refer the reader to \textcite[375--382]{gildea2018reconstructing}, who reconstructs two distinct roots serving as verbs \qu{to be} in modern Cariban languages.
...
\akuriyo \obj{eʔi} was not a potential target of innovative \obj{k-}, due to the verb being \obj{e}-initial.
...
\yukpa innovated \obj{j-} for reflexes of both \rc{a(p)} and \rc{eti}, which are preserved as auxiliaries in certain constructions \exref{yukcop}.
...
\label{sec:say}
Most reflexes of this verb are simply \obj{ka}, but a fleeting syllable \rc{ti} is reconstructed based on some imperative forms \exref{kati}.
...
It was not a potential target of \akuriyo \obj{k-}; I do not have data for its \carijo and \yukpa reflexes.
...
The view of \qu{to say} as intransitive is supported primarily by prefix patterns:
...
\label{sec:go}
This verb is reconstructed by \textcite{gildea2007greenberg} as \rc{tə(mə)}, with the second syllable only occurring in some reflexes, in certain contexts.
...
However, a comparison of selected forms with unambiguously C-initial \rc{ka(ti)} \qu{to say} suggests that an initial vowel \rc{ɨ} should be reconstructed \exref{gosaycomp}.\footnote{Many inflected forms, like e.g.\ \trio \obj{wɨtənne} or \arara \obj{wɨdolɨ} \qu{I went} \parencites[43]{triomeira1999}[153]{alves2017arara} are ambiguous, since an epenthetic \obj{ɨ} breaks up CC clusters on the prefix-verb boundary.}
...
However, the one form in \kaxui only occurs in the Progressive \exref{kaxprog.kax-82}, and \obj{j(-)} may be a reflex of the \settwo third person marker \rc{i-} and the \gl{s_a_} class marker \rc{w-}, as suggested by the occurrence of the latter with other person values \exref{kaxprog.kax-54}.
...
I have so far ignored the forms which form the basis for \posscite{gildea2007greenberg} \rc{ətepɨ}; these are compatible with my reconstruction \rc{jəpɨ}, with the addition of the detransitivizer \rc{ət(e)-}: \colorbox{come4}{\rc{ətjəpɨ}}.
In fact, the \obj{i} in the \akawaio form \obj{əsipɨ} is better explained by a sequence \rc{jə}, which has the same outcome in the \macushi reflex of \rc{jəpɨ}, \obj{ipɨ}.
...
While it seems to have resisted the extension of \rc{k-} in \PPek, \bakairi subsequently introduced it \pcref{sec:pekodian}.
It was also affected by the extension of \obj{j(i)-} in \carijo \exref{car-32}.
...
In one language, \wayana, the verb shows traits of both classes, leading me to consider it a \dbqu{mixed} verb.
...
\xe
%
Its causativized form is \obj{ɨptəka} \parencite[255]{wayanatavares2005}, containing a reflex of the transitivizer \rc{-ka}, which was restricted to \gl{s_p_} verbs \parencite{gildea2019overview}.

Comparative evidence suggests that \rc{ɨpɨtə} \qu{to go down} was originally an \gl{s_p_} verb which switched its class to \gl{s_a_} in some languages, incompletely so in \wayana.
...
As I have shown in \cref{sec:pekodian}, \PPek can be reconstructed as having the pair \rc{ipɨ} (\gl{intr}) / \rc{ɨp(ɨ)} (\gl{tr}).
Thus, while the transitive verbs in Pekodian have perfectly regular cognates in other languages of the family, intransitive \qu{to bathe} is divergent in this branch.
The reasons for this are unknown; I have not encountered \obj{i-} as a reflex of the detransitivizer in Pekodian, see also \textcite[506]{meira2010origin}.
...
\label{sec:movement}
As discussed in \cref{sec:akuriyo}, four \akuriyo \gl{s_a_} verbs are attested as not having the first person marker \obj{t͡ʃ-} in \textcite{gildea1994akuriyo}; instead, they have \obj{w-} or its phonologically reduced form Ø.
...
As for their etymology, I can only speak to \obj{erama} \qu{to return}, which is a detransitivized form of \obj{rama} \qu{to put back}, a pair which is also found in other Cariban languages.
For the other three verbs, I have not encountered potential cognates, but given their \obj{e-}initial nature and their semantics, it seems plausible that they, too, are derived from transitive verbs with meanings like \qu{to put up} etc.
