...
We analyze these forms as going back to a \PC verb of the form \rc{(ət-)jəpɨ}, morphologically segmentable into a detransitivizing prefix and a root \rc{jəpɨ}.
...
The form in question only occurs in the Progressive \exref{kaxprog.kax-82}, and it seems possible that \obj{j(-)} is actually a reflex of \rc{i-w-} \qu{\gl{3}-\gl{s_a_}-}, since the \gl{s_a_} class marker \rc{w-} is present with other person values \exref{kaxprog.kax-54}.
...
\xe
%
Most of the longer forms corresponding to \posscite{gildea2007greenberg} \rc{ətepɨ} do not show evidence for this \rc{j}, but we identify the \obj{i} in the \akawaio form \obj{əsipɨ} as a reflex of the sequence \rc{jə}, which has the same outcome in the \macushi reflex of bare \rc{jəpɨ}, \obj{ipɨ}.

Turning to the many forms seemingly reflecting \rc{əpɨ} and \rc{epɨ}, we find that both are distributed widely in the family, sometimes even co-occurring in the same language.
...
Our interpretation of the \rc{ət} portion as a detransitivizer is primarily based on its form, as well as its paradigmatically conditioned occurrence in some languages, discussed shortly.
...
While a root \rc{jəpɨ}, aforementioned sound changes, and the addition of \rc{ət-} do account for the majority of the forms in \cref{tab:come},\footnote{
Apart from aforementioned \akuriyo \obj{eepɨ}, another diachronically irregular form is \apalai \obj{oepɨ}, where the detransitivizer would have the reflex \obj{os-} \parencite[506]{meira2010origin}.
...
Alternatively, \apalai \obj{oepɨ} could be a fossilized loan from \wayana, which has replaced its reflex of \rc{ətjəpɨ}, but where regular sound changes would also have resulted in the loss of the intervocalic \rc{t} \parencite[63]{wayanatavares2005}.}
...
This was briefly discussed in \cref{sec:pekodian} for \arara (and \PPek) and in \cref{sec:taranoan} for \trio (\PTar).
...
Both reflexes of \rc{ətjəpɨ} (\trio) and of \rc{ətjəpɨ} and/or \rc{jəpɨ} (\akuriyo) resisted the introduction of \rc{t-} in \PTir.
