...
In his brief but precise discussion of \bakairi verbal person marking, \textcite{meira2003bakairi} reports the existence of two subclasses of \gl{s_a_} verbs, one taking first person \obj{w-}, and one \obj{k-}.\footnote{
...
The verb used to illustrate the first group is \obj{i} \qu{to bathe} \exref{bak-33}.
...
While \textcite[4]{meira2003bakairi} lists some \bakairi cognates of \arara verbs in \exref{arairr} as \gl{s_a_} verbs, he does not indicate whether they belong to the \gl{s_a_}-1 class with \obj{k-}, or the \gl{s_a_}-2 class with \obj{w-}.
...
However, most \ikpeng cognates of the verbs in question do not take \obj{k-}, but rather \obj{ɨ-} or Ø \exref{ikpw}.
...
While reflexes of \rc{a[p]} \qu{to be} do exist in \ikpeng, apparently only reflexes of \rc{eti} \qu{to be} occur with first person inflectional prefixes \parencite[401]{gildea2018reconstructing}.
...
Reconstructions of verb stems are deferred to \cref{sec:verbs}, but a brief comment on \qu{to come} is in order:
The stems are not fully cognate, as \ikpeng and \bakairi both show a reflex of the \PPek detransitivizer \rc{əd-} in combination with a root reconstructible as \rc{epɨ}.
...
Following \posscite[114]{meira1998proto} line of reasoning for a similar pattern in the three Taranoan languages (see also \cref{sec:taranoan}), the idiosyncratic pattern in \arara should be reconstructed to \PPek, while \bakairi and \ikpeng independently levelled the paradigm in favor of \rc{əd-epɨ}.
...
In \PWai, the \gl{1}\gl{s_p_} prefix \rc{k-} innovated in \PPar was further extended to \gl{1}\gl{s_a_}.
...
The two forms of \qu{to be} are unproblematic, whereas \qu{to go} is a special case.
...
Alternatively, this form may have been influenced by deverbalized forms of \qu{to go}, where a reflex of the \gl{s_a_} class marker \rc{w-} has become fossilized \parentext{e.g., \obj{o-\emp{w}to-topo-nho} \qu{my trip} \parencite[92]{waiwaihawkins1998}}
%
...
%\xe
%
In any case, the first person form \hixka \qu{to go} clearly points to \PWai \rc{wɨ-tom-}.
...
\textcite{meira1998proto} contributes an extensive phonological, morphological, and lexical reconstruction of \PTar, facing an interesting puzzle in the \setone paradigms of \trio and \akuriyo: \PC \gl{1}>\gl{3} \rc{t-} and \gl{1}\gl{s_a_} \rc{w-} seem to have switched places.
...
The idiosyncratic \akuriyo first person prefix \obj{ə-} on \qu{to go} is plausibly identified as a reflex of \rc{wɨ-} by 
\textcite[113]{meira1998proto}, which is supported by additional evidence:
...
For \qu{to come}, \textcite[114--115]{meira1998proto} reconstructs \PTar \rc{əepɨ} for first person, and \rc{eepɨ} for other persons, based on an idiosyncratic paradigmatic pattern in \trio and the vowel length in \akuriyo.
...
\label{sec:akuriyo}
After the split-up of \PTir, when \rc{t-} had largely replaced \rc{w-}, \akuriyo innovated a third \gl{1}\gl{s_a_} marker: \obj{k-}.
It seems to have replaced \rc{t-} only in specific environments, with the two markers showing a clear phonologically conditioned distribution in \posscite{gildea1994akuriyo} \akuriyo data \pcref{tab:aku1sa}.
...
However, since there are no examples of \obj{ə}-initial verbs with \obj{t-}, or further information about them, these cases cannot be discussed further.
...
The verbs listed for \PTir in \cref{tab:ptirverbs} in \cref{sec:taranoan} did of course also not introduce \obj{k-} in \akuriyo, although the first-person form of the copular verb \obj{eʔi} is unknown.
...
While those cases of loss were more regular, an already irregular marker undergoing idiosyncratic phonological erosion is not that surprising, see \akuriyo \rc{wɨ-} > \obj{ə-} in the preceding section.
...
The same prefix occurs on transitive verbs in \gl{3}>\gl{1} scenarios \exref{yukpatr.yuk-5}, so \obj{j(ɨ)-} is the reflex of the \PC \gl{1}S\textsubscript{(P)} marker \rc{u(j)-}.
...
\xe
%
Since \PC \gl{1}\gl{s_a_} \rc{w(ɨ)-} was extended to \gl{1}>\gl{3} scenarios in most languages \parencite[81--82]{gildea1998}, and given its inclination for phonological erosion elsewhere in the family \pcref{sec:pekodian,sec:waiwaian}, the zero marking in \gl{1}>\gl{3} scenarios can be identified as the \yukpa reflex of \rc{w-}, contrasting with \obj{j(ɨ)-}, the reflex of \rc{u(j)-}.

In intransitive verbs, this first-person zero marking is attested in a single verb, \obj{to} \qu{to go} \exref{yuk-7}.
It diverges from regular C-initial verbs, which take \obj{jɨ-}, like \qu{to sleep} \pcref{tab:yukreg}.
