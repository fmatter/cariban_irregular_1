...
We present them according to our understanding of the phonemic analysis of \bakairi by \textcites{wheatley1969bakairi}{meira2003bakairi}{meira2005bakairi} in \exref{bakverbs}.
...
Further, while there are reflexes of \rc{a[p]} \qu{to be} in \ikpeng, only \rc{eti} forms are attested with first person inflectional prefixes \parencite[401]{gildea2018reconstructing}.
...
The presence of the \ikpeng \gl{1}\gl{s_a_} marker \obj{ɨ-}/Ø suggests to us that it may be cognate with \arara \gl{1}\gl{s_a_} \obj{w(ɨ)-}.
...
Thus, we reconstruct a \gl{1}\gl{s_a_} prefix \rc{w(ɨ)-} to \PPek, identical to \arara.
...
However, the addition of a final \obj{ŋ} in \PXin is attested elsewhere: \begin{inlinelist}
...
Also, based on the fact \bakairi has generally lost much segmental material, we suggest that \bakairi \obj{gɨ} is another addition to a root \rc{ɨptə}, rather than a conservative form.
...
However, other person values show a reflex of \rc{əd-} in \arara, too \exref{ara-123}.
...
Following the line of reasoning used by \textcite[114]{meira1998proto} (see also \cref{sec:taranoan}) for a similar pattern in the three Taranoan languages, we suggest that the idiosyncratic pattern in \arara is reconstructible to \PPek, and that \bakairi and \ikpeng independently regularized the paradigm to only use \rc{əd-ebɨ}.

The V-initial nature of \qu{to go} is evidenced in its Xinguan forms, while \bakairi \phon{\rc{wɨ}}{\obj{u}} resulted in a shift of the morpheme boundary, see also \cref{sec:go}.
The \ikpeng form \obj{aran} is made compatible with our reconstruction \rc{ɨtən} by considering that \ikpeng \obj{a} is an attested outcome of \rc{ə} (\PXin \rc{o}/\obj{e}), for example \begin{inlinelist}
...
Further, both languages regularized the paradigm of \qu{to come} to \rc{əd-epɨ} and introduced \obj{k-}.
...
A formally identical prefix is found on transitive verbs in \gl{1}>\gl{3} scenarios in \hixka \exref{hixɨ.hix-17}, where it also corresponds to \waiwai \obj{w(ɨ)-} \exref{hixɨ.wai-159}.
...
\xe
%
This correspondence allows us to establish \hixka \obj{ɨ-} a reflex of \rc{wɨ-}, similar to \ikpeng \pcref{sec:pekodian}.
...
While the two roots for \qu{to be} are straightforwardly reconstructible, \qu{to go} is somewhat of a special case.
...
%
The question of how this happened in detail \parencite[107--112]{meira1998proto} still has no answer, although it seems necessary to assume a scenario whereby both \rc{t-} and \rc{w-} for a time occurred on both transitive and intransitive verbs \parencite[112]{meira1998proto}.\footnote{
...
%
As for verbs unaffected by the spread of \rc{t-}, \textcite{meira1998proto} lists the first four verbs in \cref{tab:ptirverbs} as being reconstructible to \PTar with \rc{w-}, for which we offer our reconstructed \PTir forms.
...
%
%
For \qu{to come}, \textcite[114--115]{meira1998proto} reconstructs \PTar \rc{əepɨ} for the first person, and \rc{eepɨ} for the others, based on the idiosyncratic form in \trio and the vowel length in \akuriyo.
\akuriyo (and \carijo) then levelled the original distribution of \rc{əepɨ} and \rc{eepɨ}, similar to what we have suggested for Pekodian \pcref{sec:pekodian}.
...
Further, here are two irregularly inflected \gl{s_a_} verbs in \trio, \obj{ɨhtə} \qu{to go down} and \obj{weka}/\obj{oeka} \qu{to defecate} \exref{downshit}.
...
\label{sec:carijo}
As discussed by \textcite[105--107]{meira1998proto}, \carijo has extended the \gl{1}\gl{s_p_} marker \obj{j(i)-} to \gl{s_a_} verbs.\footnote{Since almost all \gl{s_a_} verbs are V-initial, only the \envr{}{V} allomorph \obj{j-} is attested.}
...
The normal intransitive first person marker is \obj{j(ɨ)-}, including those with clear reflexes of \detrz, like \obj{otum} \qu{to wash self} in \cref{tab:yukreg}.
