\section{Inflections and verbs: innovation and resistance}
\label{sec:data}
...
As stated in \cref{sec:extensions_intro}, the six person marker extensions which did not affect all potential targets all have in common that they feature innovative first person markers on verbs that are (at least historically, in the case of loss of split-\gl{s}) members of the \gl{s_a_} class.
...
\cref{sec:pekodian} details the extension of \rc{k-} in \PPek, reflected in the three daughter languages \arara, \ikpeng, and \bakairi.
\cref{sec:waiwaian} treats the extension of \rc{k-} in \PWai, which was briefly shown in \cref{sec:extensions_intro}.
\cref{sec:taranoan} focuses on innovative \rc{t-} in \PTir, reflected in modern \trio and \akuriyo.
The topic of \crefrange{sec:akuriyo}{sec:yukpa} are innovative \gl{1}\gl{s_a_} markers only found in single languages:
...
In their comparative work, \textcite{meira2005southern} focused on phonology and lexicon, so no reconstructions of \PPek morphosyntax are found in the literature.
However, all three Pekodian languages have a regular \gl{1}\gl{s_a_} marker \obj{k-}, as evidenced by the paradigms in \cref{tab:pekreg}, which allow the reconstruction of a \PPek \gl{1}\gl{s_a_} marker \rc{k-}.
...
There is also a reflex of the copula \rc{a[p]}, serving syntactically as a postposition introducing adverbial clauses meaning \qu{if} or \qu{when} \parencite[199--201]{alves2017arara}.
...
In his brief but exact discussion of \bakairi verbal person marking, \textcite{meira2003bakairi} reports the existence of two subclasses of \gl{s_a_} verbs, one taking first person \obj{w-}, and one \obj{k-}.
...
While \textcite[4]{meira2003bakairi} states that \bakairi cognates of \arara verbs in \exref{arairr} (\obj{ge} \qu{to say}, \obj{tə} \qu{to go}, and \obj{əe[wɨ]} \qu{to come}) are \gl{s_a_} verbs, he does not indicate whether they belong to the \gl{s_a_}-1 class with \obj{k-}, or the \gl{s_a_}-2 class with \obj{w-}.%
\footnote{
...
Inflected forms can be found in \textcite{von1892bakairi}, presented in \exref{bakverbs} according to the analyses of \bakairi phonology and verbal morphology by \textcites{wheatley1969bakairi}{meira2003bakairi}{meira2005bakairi}{franchetto2016classes}.
...
All descriptions of \ikpeng, the third Pekodian language, list \obj{k-} as the only \gl{1}\gl{s_a_} marker \parencites[55]{ikpengpacheco1997}[105]{campetela1997analise}[64]{ikpengpacheco2001}[205]{alves2013verbo}.
However, most \ikpeng cognates of the verbs in question do not actually take \obj{k-}, but rather \obj{ɨ-} or Ø, as shown in \exref{ikpw}.
...
Further, while reflexes of \rc{a[p]} \qu{to be} do exist in \ikpeng, apparently only reflexes of \rc{eti} occur with first person inflectional prefixes \parencite[401]{gildea2018reconstructing}.
...
Reconstructed \PPek forms of the verbs in question are given in \Cref{tab:ppekverbs}.
Newly identified \ikpeng \obj{ɨ-}/Ø can be shown to be a reflex of \PXin \rc{w(ɨ)-} based on other (albeit irregular) cases of loss of \rc{w} \pcref{tab:pxinw}.
...
Similarly, the change of \rc{wɨ} to \bakairi \obj{u} is found in other correspondences, like \obj{udo} \parencite{meira2005southern} from \PC \rc{wɨtoto} \qu{person} \parencite[4]{gildea2007greenberg}.
Thus, a \gl{1}\gl{s_a_} prefix \rc{w(ɨ)-} can securely be reconstructed to \PPek, identical to the \arara one both in form and distribution.
In later, individual developments, \bakairi extended \obj{k-} to \qu{to go down}, and \ikpeng to \qu{to go}.

\input{floats/pxinw}

Reconstructions of verb stems, \PPek and other, are discussed in \cref{sec:verbs}, but a brief comment on \qu{to come} is in order:
The verb stems are not fully cognate, as \ikpeng and \bakairi both show a reflex of the \PPek detransitivizer \rc{əd-} in combination with a root reconstructible as \rc{epɨ}.
...
\xe
%
In contrast, \ikpeng and \bakairi show reflexes of \rc{əd-ebɨ} throughout the whole paradigm.
...
\label{sec:waiwaian}
This extension, one of the Parukotoan innovations discussed in \cref{sec:extensions_intro}, created the \hixka pattern from \cref{sec:intro}.
In \PWai, the new \gl{1}\gl{s_p_} prefix \rc{k-}, innovated in \PPar, was further extended to \gl{1}\gl{s_a_}.
...
The two forms for \qu{to be} are straightforwardly reconstructible, whereas \qu{to go} is somewhat of a special case.
...
\xe
%
In any case, this was a subsequent development, as the prefix pattern in \hixka \qu{to go} clearly shows that \PWai first person form \rc{wɨ-tom-} can be reconstructed.
...
The question of how this switch happened in detail \parencite[107--112]{meira1998proto} still has no answer, although it seems necessary to assume a scenario in which both \rc{t-} and \rc{w-} for a time occurred on both transitive and intransitive verbs \parencite[112]{meira1998proto}.\footnote{
...
Turning to verbs not affected by the spread of \rc{t-}, \textcite{meira1998proto} reconstructs four of the items in \cref{tab:ptirverbs} as taking \rc{w-} in \PTar, for which reconstructed \PTir forms are substituted here.
To this list, the second copular root \rc{eʔi} can be added, whose \trio reflex retains first person \obj{w-}.
...
Both components of the irregular change \rc{wɨ-} > \obj{ə-} -- loss of \rc{w} and lowering of \rc{ɨ} to \obj{ə} -- are found in other person prefixes \exref[exp.aku-164]{exp.aku-159}.
...
This scenario is plausible, with the exception that \trio \obj{əepɨ} is a reflex of \rc{ət-epɨ} \pcref{sec:come}, meaning that the \PTir form would have been \rc{əʔepɨ} (\PTar \rc{ət͡ʃepɨ}).
...
It seems to have replaced \rc{t-} only in specific environments, with the two markers showing a clear phonologically conditioned distribution in \posscite{gildea1994akuriyo} \akuriyo data, with all relevant verbs shown in \Cref{tab:aku1sa}.
...
He also suggests that \obj{k-} might be more recent, which is very plausible: since the distribution \envr{\rc{t-}}{\obj{ə}} / \envr{\rc{t͡ʃ-}}{\obj{e}} is reconstructible to \PTir, the most straightforward scenario is \obj{k-} replacing \rc{t-} but not \rc{t͡ʃ-} in \akuriyo.
The few \obj{t-} mentioned by \textcite{meira1998proto} were then potentially reintroduced under \trio influence.
However, since there are no examples of, or further information about, \obj{ə}-initial verbs with \obj{t-}, these cases cannot be discussed further.
...
The verbs listed for \PTir in \cref{tab:ptirverbs} in \cref{sec:taranoan} did of course also not introduce \obj{k-} in \akuriyo, although the first-person form of the copular verb \obj{eʔi} is presently unknown.
In addition, there is an \gl{s_a_} verb \obj{ɨ(h)tə} \qu{to go down}, which has an irregular first person marker \obj{p-} in, which is apparently reconstructible to \PTir \exref{tridown}.
...
Combined with the extension of \gl{2}\gl{s_a_} \obj{m-} and \gl{1+2}\gl{s_a_} \obj{kɨt-}/\obj{kɨs-} to \gl{s_p_} verbs, this created a single unified \gl{s} category for regularly inflected verbs, reflected in the paradigms in \cref{tab:carreg}.
%
\input{floats/carreg}%
%
Although the split-\gl{s} system has been lost entirely, former \gl{s_a_} verbs can be identified by the presence of a detransitivizer, like \obj{ehɨnəhɨ} \qu{to fight} \exref{car-30}, derived from \obj{hɨnəhɨ} \qu{to kill} \parencite[179]{robayo2000avance}.
...
Based on other C-initial verbs like \obj{tə} \qu{to go} or \obj{tuda} \qu{to arrive}, one would expect \obj{ka} \qu{to say} to either have conservative \obj{wɨ-} or innovative \obj{ji-}, so the zero is unexpected.
It is analysed here as a reflex of \rc{wɨ-}, primarily due to the loss of \rc{w} in \ikpeng and \hixka.
While that loss was clearly more regular, an already irregular marker undergoing idiosyncratic phonological erosion is not surprising, see \akuriyo \rc{wɨ-} > \obj{ə-} in the preceding section.
Alternatively, it is possible that the divergent development of \rc{w-} on C-initial \obj{ka} \qu{to say} and \obj{tə} \qu{to go} is a result of the latter's originally V-initial nature \pcref{sec:go}.
...
However, it does preserve the \setone prefixes in the immediate past, where a unified intransitive paradigm \pcref{tab:yukreg} is found.
...
The same prefix occurs on transitive verbs in \gl{3}>\gl{1} scenarios \exref{yukpatr.yuk-5}, meaning that \obj{j(ɨ)-} is the reflex of the \PC \gl{1}\gl{s_p_} marker \rc{u(j)-}.
...
\xe
%
Since \PC \gl{1}\gl{s_a_} \rc{w(ɨ)-} was extended to \gl{1}>\gl{3} scenarios in most languages \parencite[81--82]{gildea1998}, and since it is prone to phonological erosion elsewhere in the family \pcref{sec:pekodian,sec:waiwaian}, the zero marking in \gl{1}>\gl{3} scenarios can be identifiedn  as the \yukpa reflex of \rc{w-}, contrasting with \obj{j(ɨ)-}, the reflex of \rc{u(j)-}.

In intransitive verbs, this first-person zero marking is attested in a single verb, \obj{to} \qu{to go} \exref{yuk-7}.
It diverges from regular C-initial verbs, which take \obj{jɨ-}, like \qu{to sleep} in \cref{tab:yukreg}.
It is thus the only verb attested as having resisted the extension of \obj{j-} in \yukpa.
