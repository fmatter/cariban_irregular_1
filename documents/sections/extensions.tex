...
For the reconstructed forms of the \PPek verb stems we refer to \cref{sec:verbs}, but a brief comment on \qu{to come} is in order:
...
Following the line of reasoning used by \textcite[114]{meira1998proto} for a similar pattern in the three Taranoan languages (see also \cref{sec:taranoan}), we suggest that the idiosyncratic pattern in \arara is reconstructible to \PPek, and that \bakairi and \ikpeng independently regularized the paradigm to only use \rc{əd-epɨ}; this verb shows similar throughout the family \pcref{sec:come}.
...
Further, both languages regularized the paradigm of \qu{to come} to \rc{əd-epɨ}, accompanied by the introduction of first person \obj{k-}.

\subsection{\PWai \rc{k-}}
\label{sec:waiwaian}
This person marker extension led to the \hixka pattern from \cref{tab:hixintro} in \cref{sec:intro} and was one of the Parukotoan extensions discussed in \cref{sec:extensions_intro}.
...
\xe
%
This correspondence allows us to establish \hixka \obj{ɨ-} as a reflex of \rc{wɨ-}, with a similar phonological reduction as in \ikpeng \pcref{sec:pekodian}.
...
We take a conservative stance and reconstruct it to \PTir only, but acknowledge the possibility of it taking place already in \PTar; this decision is irrelevant for our analysis.}

\input{floats/ptirverbs}

Turning to verbs not affected by the spread of \rc{t-}, \textcite{meira1998proto} reconstructs the first four items in \cref{tab:ptirverbs} as taking \rc{w-} in \PTar, for which we provide our reconstructed \PTir forms.
To this list, the other copular root \rc{eʔi} can be added, whose \trio reflex retains first person \obj{w-}.
...
Both \akuriyo and \carijo then levelled this original distribution, similar to what we have suggested for the Pekodian languages \pcref{sec:pekodian}.
We agree with this scenario, with the exception that we identify \trio \obj{əepɨ} as a reflex of \rc{ət-epɨ} \pcref{sec:come}, meaning that the \PTir form would have been \rc{əʔepɨ} (\PTar \rc{ət͡ʃepɨ}).
...
Given that this data potentially has strong \trio and/or \wayana influence \parencite[253]{gildea1998} and the lack of support by the available part of \posscite{meira1998proto} data, we cannot confidently reconstruct these verbs as resisting the extension of \PTir \rc{t-}.
...
He also suggests that \obj{k-} might be more recent, with which we agree: since the distribution \envr{\rc{t-}}{\obj{ə}} / \envr{\rc{t͡ʃ-}}{\obj{e}} is reconstructible to \PTir, the most straightforward scenario is \obj{k-} replacing \rc{t-} but not \rc{t͡ʃ-} in \akuriyo.
The few \obj{t-} mentioned by \textcite{meira1998proto} were potentially reintroduced under \trio influence.
However, since there are no examples of, or further information about, \obj{ə}-initial verbs with \obj{t-}, we cannot discuss these cases.
...
The verbs listed for \PTir in \cref{tab:ptirverbs} in \cref{sec:taranoan} did of course also not introduce \obj{k-} in \akuriyo, although we do not know the first-person form of the copular verb \obj{eʔi}.
In addition, there is an \gl{s_a_} verb \obj{ɨ(h)tə} \qu{to go down}, which has an irregular first person marker \obj{p-} in, likely inherited from \PTir \exref{tridown}.
...
Combined with the extension of \gl{2}\gl{s_a_} \obj{m-} and \gl{1+2}\gl{s_a_} \obj{kɨt-}/\obj{kɨs-} to \gl{s_p_} verbs, this created a single unified \gl{s} category for regularly inflected verbs, seen in the paradigms in \cref{tab:carreg}.
%
\input{floats/carreg}
%
...
We analyse it as a reflex of \rc{wɨ-}, primarily based on the (albeit more regular) loss of \rc{w} in \ikpeng and \hixka, and the presumedly higher propensity of an irregular marker for idiosyncratic phonological erosion.
