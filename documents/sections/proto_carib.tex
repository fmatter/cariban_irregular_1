...
A specific aspect of the system, the \gl{s_a_} vs \gl{s_p_} distinction, plays a role in incomplete extensions and is discussed in \cref{sec:split}.
...
\gl{s_a_} verbs took similar markers as the \gl{a}-oriented ones in transitive verbs, with the exception of first person (\gl{1}\gl{s_a_} \rc{w-} vs \gl{1}>\gl{3} \rc{t(i)-}) and the absence of \rc{i} after all \gl{s_a_} prefixes.
On the other hand, \gl{s_p_} verbs took markers fully identical to the \gl{p}-oriented ones, with \gl{3}\gl{s_p_} \rc{n(i)-} aligning with \gl{3}>\gl{3} scenarios.
...
The main mechanism of these changes are \textsc{person marker extensions}, person prefixes being extended to verbal paradigm cells previously occupied by other prefixes.
%\footnote{
...
For \textcite{gildea1998}, person marker extensions are relevant for the loss of split-\gl{s} and the accompanying changes to indexing alignment, whereas this study focuses on a different aspect.
...
The third argument in favor of the lexical diffusion scenario goes back to conservative forms like in \hixka and \trio.
...
\footnote{As an honorable mention, when \ikpeng replaced third person \setone with \settwo prefixes, \obj{a} \qu{to be} and \obj{ke} \qu{to say} retained \obj{n-} \parencite[12]{matter2019arara}.
	However, the spread of the innovative markers had an entirely different dynamic, spreading from subordinate to main clauses.} either occurred with other person values and/or targeted \gl{s_p_} verbs.
...
This will also clarify how the \gl{s_a_}/\gl{s_p_} distinction can be lost for a single person, or how \gl{s_p_} verbs can take on \gl{s_a_} markers with apparent semantic impunity.

\input{floats/completed}

\subsection{The Cariban split-\gl{s} system}
\label{sec:split}
As seen in \cref{sec:pc_person}, the split between \gl{s_a_} and \gl{s_p_} verbs was instantiated by inflection classes within the \PC \setone person paradigm, but this was not the only difference:
...
Also, \gl{s_p_} verbs took the \gl{2}\gl{s_p_} prefix \rc{ə(j)-} in imperatives, while \gl{s_a_} verbs were unprefixed \parencites[89]{gildea1998}[208]{meira2000split}.
...
\footnote{In the \PC transitive verb template, derivational prefixes were in a paradigmatic relationship with the third person marker \rc{i-}:
...
In the next section, it will be seen that many of the verbs not affected by person marker extensions belong to the small group of \gl{s_a_} verbs without a reflex of \detrz.
