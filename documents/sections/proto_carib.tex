...
Person indexation in transitive verbs can be described as being conditioned by a basic hierarchy \fbox{\gl{1}/\gl{2} > \gl{3}}.
...
They contrast with regular \gl{sa} verbs, which have innovative first person markers in both languages.
...
%\end{enumerate}
%
All innovations are person marker extensions except the innovative first person P form, which consists of a free pronoun combined with the linker \rc{j-} for V-initial stems.
\cref{fig:par_ext} shows the extensions in bold and reproduces \posscite{gildea1998} tables as a tree diagram, with adapted transcription and an additional \kaxui \gl{1}\gl{sp} marker ∅/\obj{j-} \perscommpar{Spike Gildea}.
%
\begin{figure}[hbt]
	\centering
...
\footnote{In the \PC verb template, derivational prefixes were in a paradigmatic relationship with the earlier third person marker \rc{i-}:
