...
\textcite{gildea1998} discusses person marker extensions in the context of the loss of the split-\gl{s} system and the accompanying changes to indexing alignment, but a particular aspect of these extensions is also relevant for conservatively inflected intransitive verbs.
...
The continued presence of the old \gl{1}\gl{s_a_} prefix in a few verbs is best interpreted as the extension stopping short of these verbs, rather than affecting all targets (\gl{s_a_} verbs).
...
The markedly different behavior of \gl{s_a_} and \gl{s_p_} verbs with regards to the extent of extensions affecting them points to the split-\gl{s} system playing some role, so its main properties will be discussed in \cref{sec:split}.
