...
The irregular first person prefixes introduced in \cref{sec:intro} are relics, inherited from the ancestral \PC system \pcref{sec:pc_person}.
That system was subject to many different kinds of innovations; responsible for the patterns discussed are person marker extensions not spreading through the entire \gl{s_a_} lexicon \pcref{sec:extensions_intro}.
A particular component of the system, the \gl{s_a_} vs \gl{s_p_} distinction, plays a role in what extensions are incomplete and is discussed in \cref{sec:split}.
...
That is, \gl{s_a_} verbs took similar markers as the \gl{a}-oriented ones in transitive verbs, with the exception of first person (\gl{1}>\gl{3} \rc{t(i)-} vs \gl{1}\gl{s_a_} \rc{w-}) and the absence of \rc{i} after all \gl{s_a_} prefixes.
...
The main mechanism of change leading to these modifications are \textsc{person marker extensions}, i.e. verbal person prefixes being extended to paradigm cells previously occupied by other prefixes.
%\footnote{
...
\textcite{gildea1998} illustrates this using the three Parukotoan languages as an example.
...
\textcite{gildea1998} discusses person marker extensions relating to the loss of split-\gl{s} and the accompanying changes to indexing alignment, but a different aspect of these extensions is relevant for this study.
Namely, they most likely took place via lexical diffusion, characterized as a type of extension by \textcite[106--115]{harris1995historical}; this hypothesis is supported by three facts.
...
Second, while there is no detailed diachronic scenario for the switch of \gl{1}>\gl{3} \rc{t-} and \gl{1}\gl{s_a_} \rc{w-} in the Tiriyoan languages \pcref{sec:taranoan}, \textcite[111--112]{meira1998proto} convincingly argues that it must have happened gradually rather than instantaneously, and entailed both markers spreading at the same time.
...
The continued presence of the old \gl{1}\gl{s_a_} prefix in a few verbs is best interpreted as the extension stopping short of these verbs, rather than spreading through the entire \gl{s_a_} lexicon.
...
\footnote{As an honorable mention, when \ikpeng introduced a new mixed person paradigm to \setone constructions, \obj{a} \qu{to be} and \obj{ke} \qu{to say} retained third person \obj{n-} \parencite[12]{matter2019arara}.
	However, the spread of the innovative third person markers had an entirely different pathway from the extensions discussed here, spreading from subordinate to main clauses, rather than from verbs to verbs.} either occurred with other person values and/or targeted \gl{s_p_} verbs.
...
This starkly different behavior of \gl{s_a_} and \gl{s_p_} verbs when subject to extensions points to the split-\gl{s} system playing some role, so its main properties will be discussed in \cref{sec:split}.
This will also explain why it is possible for the \gl{s_a_}/\gl{s_p_} distinction to be lost only for a single person, or for \gl{s_p_} verbs to take on \gl{s_a_} markers with apparent semantic impunity.
...
%
The distinction between \gl{s_a_} and \gl{s_p_} was also borne out in imperatives, where the latter took the \gl{p}-oriented second person prefix \rc{ə(j)-} while the former received no prefix \pcref{tab:imperatives}.
...
In the modern languages maintaining the split-\gl{s} system, mismatches between the semantics of verbs and their \gl{s_a_} or \gl{s_p_} membership are common, exemplified with \kalina data in \exref{kar2}.
%
