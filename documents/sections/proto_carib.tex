...
A particular component of the system, the distinction between \gl{s_a_} and \gl{s_p_} verbs, plays a role in what extensions are incomplete and is the topic of \cref{sec:split}.
...
\textcite{gildea1998} discusses person marker extensions in the context of the loss of the split-\gl{s} system and the accompanying changes to indexing alignment, but this study focuses on a particular aspect of theirs.
...
Readers will also get an idea how it is possible for the \gl{s_a_}/\gl{s_p_} distinction to be lost only for a single person, or for \gl{s_p_} verbs to take on \gl{s_a_} markers with apparent semantic impunity.

\input{floats/completed}

\subsection{The Cariban split-\gl{s} system}
\label{sec:split}
As seen in \cref{sec:pc_person}, the distinction between \gl{s_a_} and \gl{s_p_} verbs was instantiated by two inflection classes within the \PC \setone person paradigm, but this was not the only inflectional difference:
%\begin{inlinelist}
...
\xe
%
\obj{ekema} \qu{to be afraid} takes an \gl{a}-oriented marker, since it is an \gl{s_a_} verb \exref{kar2.kar-59}, while the \gl{s_p_} verb \obj{awomɨ} \qu{to get up} takes a \gl{p}-oriented marker \exref{kar2.kar-61}.
...
Regarding the form of this marker, \textcite[505--512]{meira2010origin} reconstruct two distinct prefixes for \PC: reciprocal \rc{əte-} and reflexive \rc{e-}, although on verbs their reflexes have been merged in a single morpheme in modern languages.
