...
\input{floats/hix-intro}
\input{floats/tri-intro}
...
This is illustrated for \hixka in \cref{tab:hix-intro},\footnote{All forms under discussion have prefixes from \posscite[16-18]{gildea1998} "Set I" paradigm, which is conditioned by \gl{tam} suffixes.
...
Standard IPA symbols are used for transcribing language data, except for coronal rhotics, which are simply represented with \ort{r}, rather than \ort{ɽ} for \wayana or \ort{ɾ̠} for \maqui etc.
...
A similar pattern exists in \trio \pcref{tab:tri-intro}, where \gl{sa} verbs have a prefix with phonologically conditioned allomorphs \envr{\obj{t-}}{\obj{ə}} and \envr{\obj{s-}}{\obj{e}}, while the verb \qu{to go} has a first-person prefix \obj{wɨ-}.
...
In \cref{sec:background}, relevant aspects of the \PC verbal system are introduced, and it is shown that the mechanism of person marker extensions is responsible for patterns like in \cref{tab:hix-intro,tab:tri-intro}.
